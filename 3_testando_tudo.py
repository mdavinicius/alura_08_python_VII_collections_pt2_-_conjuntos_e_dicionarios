...
aparicoes = Counter(texto1.lower())  # conta quantas letras apareceram no texto com Counter()
print(aparicoes)
print("-----------------------------------")

total_de_caracteres = sum(aparicoes.values())  # conta quantos caracteres tem no texto total com sum() somando todos os valores do dicionário
print(f"Total de caracteres: {total_de_caracteres}")
print("-----------------------------------")

for letra, frequencia in aparicoes.items():  # calcula a frequencia que cada caracter apareceu no texto. Ex: 'a' = 10 em 1000, logo 1%
    tupla = (letra, (frequencia / total_de_caracteres) * 100)
    print(tupla)
print("-----------------------------------")

lista_frequencia = [(letra, (frequencia / total_de_caracteres) * 100) for letra, frequencia in aparicoes.items()]
print(lista_frequencia)
lista_frequencia = Counter(dict(lista_frequencia))
print(lista_frequencia.most_common(10))  # calcula os caracteres top 10 de frequencia com most_common(10)
print("-----------------------------------")

# agora vamos criar uma função e tornar isso mais apresentavel
def analista_frequencia_de_letras(texto):
    aparicoes = Counter(texto.lower())
    total_de_caracteres = sum(aparicoes.values())
    print("O total de caracteres é {}.\nOs top 10 mais comuns são:".format(total_de_caracteres))
    lista_frequencia = [(letra, (frequencia / total_de_caracteres) * 100) for letra, frequencia in aparicoes.items()]
    lista_frequencia = Counter(dict(lista_frequencia))
    mais_comuns = lista_frequencia.most_common(10)
    for caractere, frequencia in mais_comuns:
        print("{} => {:.2f}%".format(caractere, frequencia))


analista_frequencia_de_letras(texto1)
print("-----------------------------------")
analista_frequencia_de_letras(texto2)
